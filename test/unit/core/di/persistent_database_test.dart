...
void main() {
  group('Persistent Database Implementation (Issue #113 Phase 3)', () {
    group('Database Persistence Tests', () {
      test('should create persistent database connection in production', () async {
        ...
        // 期待: 永続化データベースが作成される
        expect(() async {
          final connection = await container.createPersistentDatabaseConnection();
          return connection;
      ...
      test('should use path_provider for database file location', () async {
        ...
        expect(() async {
          final databasePath = await container.getDatabaseFileWithPathProvider();
          return databasePath;
        }, throwsA(isA<UnimplementedError>()));
      });
      ...
      test('should maintain data persistence across app restarts', () async {
        ...
        expect(true, isFalse, reason: 'Persistent database not yet implemented');
      });
    });
    ...
    group('Environment-based Database Selection', () {
      ...
      test('should use persistent database in production environment', () async {
    ...
    group('Migration and Compatibility Tests', () {
      test('should handle migration from memory to persistent database', () async {
        // 🔴 Red: メモリDBから永続化DBへの移行処理
        
        expect(true, isFalse, reason: 'Migration logic not implemented');
      });

      test('should preserve existing data structure in persistent mode', () async {
        // 🔴 Red: 既存のデータ構造が永続化でも保持される
        
        expect(true, isFalse, reason: 'Data structure preservation not verified');
      });
    });
  });
}