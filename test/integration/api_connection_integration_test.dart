...
void main() {
  group('実際のAPI接続統合テスト', () {
    ...
    test('実際のHotPepper APIキーでの接続テスト', () async {
      printOnFailure('🔍 実際のAPIキーでの接続テストを開始...');

      // API診断実行
      final diagnostics =
          await ApiDiagnostics.getComprehensiveDiagnostics(forceRefresh: true);
      printOnFailure('📋 診断結果:');
      printOnFailure(diagnostics.toString());
    ...
    test('実際のAPI呼び出しテスト', () async {
      printOnFailure('🔍 実際のAPI呼び出しテストを開始...');

      // API接続テスト実行
      final result = await ApiConnectionTester.testActualApiCall();
      printOnFailure('📋 接続テスト結果: ${result.toString()}');

      if (!result.isSuccessful) {
        printOnFailure('❌ エラー詳細: ${result.errorMessage}');
        printOnFailure('⏱️ 実行時間: ${result.duration.inMilliseconds}ms');
      }
    ...
    test('HotpepperApiDatasourceImplでの実際のAPI呼び出し', () async {
      printOnFailure('🔍 HotpepperApiDatasourceImplでの実際のAPI呼び出しテスト...');
      ...
      try {
        ...
        printOnFailure('📋 検索結果:');
        printOnFailure('- 利用可能件数: ${searchResult.resultsAvailable}');
        printOnFailure('- 返却件数: ${searchResult.resultsReturned}');
        printOnFailure('- 開始位置: ${searchResult.resultsStart}');
        printOnFailure('- 店舗数: ${searchResult.shops.length}');

        if (searchResult.shops.isNotEmpty) {
          final shop = searchResult.shops.first;
          printOnFailure('- 最初の店舗: ${shop.name}');
          printOnFailure('- 住所: ${shop.address}');
          printOnFailure('- ジャンル: ${shop.genre}');
        }

        expect(searchResult.resultsAvailable, greaterThan(0),
            reason: '検索結果が1件以上ある必要があります');
        expect(searchResult.shops, isNotEmpty, reason: '店舗リストが空でない必要があります');
      } catch (e, stackTrace) {
        printOnFailure('❌ API呼び出しエラー: $e');
    ...
    test('位置指定でのAPI呼び出しテスト', () async {
      printOnFailure('🔍 位置指定でのAPI呼び出しテスト（東京駅周辺）...');
      ...
      try {
        ...
        printOnFailure('❌ 位置指定API呼び出しエラー: $e');
        printOnFailure('スタックトレース: $stackTrace');
